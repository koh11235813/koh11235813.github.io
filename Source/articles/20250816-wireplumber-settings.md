...
wireplumberの設定ファイルの書き方が 0.4.xと 0.5.xでは異なります(fn-0)．
...
`10-null-sink.conf`は，gitlabのpipewireの設定例(fn-1)そのままです．
```conf
context.objects = [
    {   factory = adapter
        args = {
...
//footnote[fn-0][https://pipewire.pages.freedesktop.org/wireplumber/daemon/configuration/migration.html#config-migration]
//footnote[fn-1][https://gitlab.freedesktop.org/pipewire/pipewire/-/wikis/Virtual-Devices#single-nodes]
...
$ pw-top
```
//image[pw-link][実行結果][width=140mm]
...
wireplumberは0.5.xから設定の書き方が変更され，ユーザー設定の場所も変更されました(fn-2)．設定ファイルは，`~/.config/wireplumber/wireplumber.conf.d`，スクリプトは，`~/.local/share/wireplumber/scripts`に配置します(fn-3)．

まずはスクリプトを読みこませる設定ファイルを作成します(fn-4)．
```conf
...
//footnote[fn-2][https://pipewire.pages.freedesktop.org/wireplumber/daemon/configuration/migration.html]
//footnote[fn-3][https://pipewire.pages.freedesktop.org/wireplumber/daemon/locations.html]
//footnote[fn-4][https://pipewire.pages.freedesktop.org/wireplumber/scripting/custom_scripts.html]
...
この中の，`node.name`の`alsa_output.pci-0000_03_00.1`がこのデバイス名です．これを消す設定を書きます．
...
`~/.config/pipewire/pipewire.conf.d/10-rate.conf`のような設定を作成します(fn-5)．
```
...
元の設定ファイル(fn-6)のデフォルトサンプルレートを変更することで，サンプルレートを変更します．
...
//footnote[fn-5][https://gitlab.freedesktop.org/pipewire/pipewire/-/wikis/Guide-Rates]
//footnote[fn-6][https://gitlab.freedesktop.org/pipewire/pipewire/-/wikis/Config-PipeWire]
...
ちなみに，私は(fn-0)のバージョンごとの設定の書き方，置く場所の違いに特に混乱しました．みなさんもお気をつけください．
